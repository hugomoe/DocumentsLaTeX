...
\end{proof}
Si on définit $D_d$ l'opérateur de "dérivation discrète" par $D_d f(x)=\frac{f(x+\frac{d}{2})-f(x-\frac{d}{2})}{d}$ et on pose $F^{n+1}(x)= \int_{-\infty}^{x}F^{n}(y)dy$ où $F^{0}(x)= f(x)$, on à alors le lemme suivant 
