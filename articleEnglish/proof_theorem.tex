...
First some useful properties go homographies will be recalled. The link between projective space and homograhies will not be used here. An homography is a bijective map of form 
...
$H$ is invertible since $h$ is, $H$ is not unique because $\lambda H$ correspond to the same homography for all $\lambda \in \mathbb{R}_+$.
...
Let's denote $\sim$ the equivalence relation on $GL_{3}(\mathbb{R})$ defined by \[A\sim B \iff \exists \lambda\in \mathbb{R}^{*} , A=\lambda B\] i.e $A\equiv B$ if and only if they defined the same homography.
...
Let $h$ be an homography and $H$ a matrix associated to it, assuming $\det (H)=1$ without loss of generality. Let's prove there exists some parameter $(\theta,\phi,\psi,\delta,\delta',\cbf,\xbf_v)$ such that
\begin{equation*}
h= \tau_{\cbf} \circ z_{\frac{\delta}{\delta'}}\circ R_{\psi} \circ h_{\theta,\delta'} \circ R_{\phi} \circ \tau_{\xbf_v}
\end{equation*}

Since every transform in the decomposition are homographies, it can be written with matrix. So we want $(\theta,\phi,\psi,\delta,\delta',\cbf,\xbf_v)$ such that
 \begin{equation*}
...
\end{pmatrix}
\end{equation*}
First the translations are determined. Let $(x_1 , y_1 )$ and $(x_2 , y_2 )$ be two vectors and $H_t$ the matrix $T_{-(x_2 , y_2 )}  \cdot H \cdot T_{(x_1 , y_1 )}$. Let's determined  $(x_1 , y_1 )$ and $(x_2 , y_2 )$. It exists $(\theta,\phi,\psi,\delta,\delta')$ such that $H_t=Z_{\frac{\delta}{\delta'}} \cdot R_{\psi} \cdot H_{\theta,\delta} \cdot R_{\phi}$.
