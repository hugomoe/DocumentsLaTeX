...
\ssse{Building the fourth integral image}
 ...
 \begin{algorithm}[H]
 \caption{$build4Integrale(img)$, which returns the four first integrals of $img$ (described in \ref{4Integral})}
 ...
 \ssse{Convolution using the 4-integral image}
  ...
  \begin{algorithm}[H]
 \caption{$convolImg(img,Img,xy,d)$, which convolves the image with $\Gcal_3^d * \Gcal_1^1$(define by \ref{formule_convol_n_int}) (described in \ref{4Integral})}
...
\tcc{Computation of the third discret derivatives to convolve with the convolution of three boxes}
 ...
 \end{algorithm}
  ...
  \begin{algorithm}[H]
 \caption{$eval4Integral(img,Img,x)$, which evaluates in any given real point (described in \ref{4Integral})}
