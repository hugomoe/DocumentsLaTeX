...
\subsubsection{Principe général}
	...
	\red{(insérer une figure explicative)}
	...
	On est ainsi capable d'effectuer, en trois étapes $\mathcal R$ et sans aliasing, tous les \emph{shears}. Or une affinité se décompose toujours en deux \emph{shears}, on peut donc toujours traiter une affinité quelconque, et ce, sans aliasing.
...
\subsubsection{Étapes de l'algorithme}
	On détaille donc ici les étapes de l'algorithme de traitement d'une affinité par cette méthode multi-étape. Le pseudo-code correspondant est présent en annexe (\red{référence à ajouter}).
	...
	\paragraph{Décomposition de $A$}
		...
		\begin{figure}
		\centering
		\subfigure[Spectre de l'image d'entrée]{\includegraphics[width=37mm]{uMax_vMax_spectreEntree.png}}
		\subfigure[Spectre de l'image de sortie]{\includegraphics[width=37mm]{uMax_vMax_spectreSortie.png}}
		\subfigure[Spectre de l'image d'entrée]{\includegraphics[width=47mm]{uMax_vMax_spectreUtile.png}}
		...
		Pour des raisons géométriques, on peut toujours réduire les sur-échantillonnages à $r_h \leq 3$ et $r_v \leq 3$ (au delà, on \red{insère un schéma explicatif du pourquoi du comment}).
