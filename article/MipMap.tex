...
% parler Ã©ventuellement de la mÃ©thode naÃ¯ve, ou d'autres mÃ©thodes actuelles qui ne fonctionnent pas
  Afin de traiter les homographies plusieures méthodes ont été développées. Cette partie présente de manière non-exhaustive les méthodes déjà existantes (méthode naïve, Mipmap et Ripmap). Elles seront comparées avec notre nouvelle méthode dans les expériences.
...
Un exemple est représenté figure \ref{MipMap_real}. Un pseudo-code naif figure en annexe, c'est l'algorithme \ref{buildMipmap}.
...
Quand on cherche la valeur d'un pixel de l'image d'arrivée, on approche la zone de l'image de départ à laquelle il correspond à l'aide des différentielles de l'homographie inverse. On obtient alors un parallélogramme, qu'on essaye d'approcher avec des carrés. 
...
La figure \ref{intertrilineaire} représente cette interpolation. Elle est décrite en annexe dans les algorithmes \ref{intertri1} et \ref{intertri2}
...
Pour tenter de résoudre ce problème on utilise un Ripmap \cite{akenine2008real}. C'est en fait un Mipmap où l'on a aussi calculé la moyenne des pixels de tous les rectangles dont les côtés sont des puissances de deux. On en présente un exemple dans la figure \ref{Ripmap_real}, et deux algorithme de construction en annexe en  \ref{pseudo_code_Ripmap}. L'algorithme 

Ainsi la fonction de distance ne renvoie plus une seule valeur mais une pour chaque côté du rectangle. On réalise alors une interpolation quadrilinéaire (bilinéaire entre les étages et bilinéaire dans chacun d'eux). Elle est représentée dans la figure \ref{interbibilineaire}. L'algorithme \ref{interbibi1} disposé en annexe en \ref{pseudo_code_Ripmap} décrit cette interpolation. L'algorithme \ref{buildRipmap1} est naif, en pratique on a implémenté l'algorithme \ref{buildRipmap2}.
